const checkInsertPR = (req,res,next) => {
    ...
    try {
        ...
        if(category_id ===  0) throw new Error("category tidak boleh kosong atau berbentuk string")
        if(stock === 0) throw new Error("stock tidak boleh kosong atau berbentuk string")
...
const checkUpdatePR = (req,res,next) => {
    ...
    try {
        if(name === "") throw new Error("nama tidak boleh kosong atau integer")
        if(category_id === 0) throw new Error("category tidak boleh kosong atau berbentuk string")
        if(typeof stock === "string") throw new Error("stock tidak boleh berbentuk string")
        if(price === 0) throw new Error("price tidak boleh kosong")
        if(category_name === 0) throw new Error("category tidak boleh kosong")
    } catch (err){
        return res.send(`${err}`)
